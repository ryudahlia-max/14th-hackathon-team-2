...
class ChatServiceTest {
    private ChatRoomRepository rooms = mock(ChatRoomRepository.class); private ChatRoomMemberRepository members = mock(ChatRoomMemberRepository.class); private ChatMessageRepository messages = mock(ChatMessageRepository.class); private MessageReactionRepository reactions = mock(MessageReactionRepository.class); private FakeCoreAccess core = new FakeCoreAccess(); private RecordingRealtime realtime = new RecordingRealtime(); private MediaStoragePort storage = mock(MediaStoragePort.class); private ChatService service;
    @BeforeEach void setUp() { when(storage.temporaryDownloadUrl(anyString())).thenAnswer(invocation -> invocation.getArgument(0)); service = new ChatService(rooms, members, messages, reactions, core, realtime, storage); }
    ...
    static final class FakeCoreAccess implements CoreAccessPort { boolean friends; public boolean areAcceptedFriends(UUID a, UUID b) { return friends; } public boolean isGroupMember(UUID u, UUID g) { return false; } public List<UUID> getGroupMemberIds(UUID g) { return List.of(); } public Optional<MissedRoutineOccurrence> getMissedRoutineOccurrence(UUID occurrenceId, UUID targetUserId) { return Optional.empty(); } public Optional<UserSummary> getUserSummary(UUID userId) { return Optional.empty(); } public Optional<GroupSummary> getGroupSummary(UUID groupId) { return Optional.empty(); } public boolean hasAiImageConsent(UUID userId){return false;} }
