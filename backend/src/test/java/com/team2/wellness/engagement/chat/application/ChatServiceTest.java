...
class ChatServiceTest {
    ...
    static final class FakeCoreAccess implements CoreAccessPort { boolean friends; public boolean areAcceptedFriends(UUID a, UUID b) { return friends; } public boolean isGroupMember(UUID u, UUID g) { return false; } public List<UUID> getGroupMemberIds(UUID g) { return List.of(); } public Optional<MissedRoutineOccurrence> getMissedRoutineOccurrence(UUID occurrenceId, UUID targetUserId) { return Optional.empty(); } public Optional<UserSummary> getUserSummary(UUID userId) { return Optional.empty(); } public boolean hasAiImageConsent(UUID userId){return false;} }
