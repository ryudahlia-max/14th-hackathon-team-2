...
export default function FriendRoutinePage() {
  ...
  const load = useCallback(async () => {
    try {
      const [feed, reactions] = await Promise.all([getFeed(), getReceivedReactions()]);
        ...
        <div className="space-y-5">
          {tab === 'friendRoutine' ? items.map(item => {
            ...
            return <div key={item.completionId} className="flex gap-3">
              <Avatar friendId={item.userId} className="size-8" />
          ...
          }) : received.map(item => { const reaction = fromApiType(item.type) ?? 'heart'; const Icon = ICONS[reaction]; return <div key={`${item.completionId}-${item.reactorId}`} className="flex gap-3"><Avatar friendId={item.reactorId} className="size-8" /><p className="flex-1 text-xs"><b>{item.reactorNickname}</b>님이 <b>{item.routineTitle}</b>에 공감을 남겼습니다. <span className="text-gray-400">{formatRelativeTime(item.createdAt)}</span></p><Icon size={17} className={COLORS[reaction]} /></div>; })}
